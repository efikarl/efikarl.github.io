...
> 同余：记作：$a≡b\pmod m$
> 
> 欧拉函数：\[ φ(n) = n(1-\frac{1}{p_1})(1-\frac{1}{p_2})...(1-\frac{1}{p_k}) \]
> 欧拉定理：\[ 若：a,m ∈ N+ \quad and \quad (a,m) = 1 \quad 则：a^{φ(m)}≡{1} \pmod {m} \]
> 
证明：欧拉定理

假设：
\[ B = \{ \quad b_i ∈ [1, m) \quad and \quad (b_i, m) = 1 \quad \} \]

如果：
\[ a_i = a * b_i \]

那么：
\[ A = \{ \quad a_i ∈ [1, m) \quad and \quad (a_i, m) = 1 \quad \} \]

因此：
\[ a^{φ(m)} \prod_{i=1}^{φ(m)}{b_i} ≡ \prod_{i=1}^{φ(m)}{a*b_i} ≡ \prod_{i=1}^{φ(m)}{a_i} ≡ \prod_{i=1}^{φ(m)}{b_i} \pmod {m} \]

故：
\[ m | a^{φ(m)} \prod_{i=1}^{φ(m)}{b_i} - \prod_{i=1}^{φ(m)}{b_i} \]

即：
\[ m | (a^{φ(m)} - 1) \prod_{i=1}^{φ(m)}{b_i} \]

又：
\[ (m,\prod_{i=1}^{φ(m)}{b_i}) = 1 \]
...
- [x] [pkcs#1](https://github.com/efikarl/rust-rsa.git)
